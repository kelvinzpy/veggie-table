...
def vidPlayback(screen, num, datasheet, LFoodList, RFoodList, sampleNum, txtfile):
    ...
    while running:
      ...
      if ret == True:
        ...
        screen.blit(frame, (((windowW - videowidth)/2),0))
        pygame.display.update()
        ...
        for event in pygame.event.get():
            ...
            if (event.type == KEYDOWN and event.key == K_LEFT) or (event.type == KEYDOWN and event.key == K_a):
                ...
                recordIndicator(screen, (leftborder + 2 + leftcount*13), min((videoheight + 5), (windowH - 45)))
                entry[2] += "0"
                leftcount += 1
                pass
            ...
            if event.type == KEYDOWN and event.key == K_RIGHT or (event.type == KEYDOWN and event.key == K_d):
                ...
                recordIndicator(screen, (rightborder - 9 - rightcount*13), min((videoheight + 5), (windowH - 45)))
                entry[2] += "1"
                rightcount += 1
                pass            
